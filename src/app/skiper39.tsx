...
const CrowdCanvas = ({ src, rows = 15, cols = 7 }: CrowdCanvasProps) => {
  ...
  useEffect(() => {
    ...
    const normalWalk = ({ peep, props }: { peep: any; props: any }) => {
      ...
      // Increased xDuration to slow down horizontal movement
      const xDuration = 25;
      const yDuration = 0.25;

      const tl = gsap.timeline();
      // Reduced timeScale to make the overall animation slower
...
const Skiper39 = () => {
  return (
    <div className="relative h-full w-full bg-white text-black">
      <div className="top-32 absolute left-1/2 grid w-full -translate-x-1/2 content-start justify-items-center gap-6 text-center z-10 px-6 mt-6">
        <h1 className="font-editorial text-4xl md:text-[54px] text-carbon tracking-display leading-display">
          Anyone can make a <br className="hidden md:block"/> great video. <span className="text-green-500">That means you.</span>
        </h1>
